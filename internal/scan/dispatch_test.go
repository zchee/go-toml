...
func runDispatchSmoke(t *testing.T) {
	...
	for _, c := range dispatchSmokeCases {
		t.Run(c.name, func(t *testing.T) {
			...
			if want := c.want["ScanBasicStringEscape"]; ScanBasicStringEscape(c.input) != want {
				t.Errorf("ScanBasicStringEscape(%q) = %d, want %d", c.input, ScanBasicStringEscape(c.input), want)
			}
...
func pinAllDispatch(t *testing.T,
	bareKey, basicString, basicStringEscape, basicStringStrict, commentBody, bareValueEnd, countLines, literalString, ws, newline, utf8 func([]byte) int,
) {
	t.Helper()
	swapDispatch(t, &scanBareIdent, bareKey)
	swapDispatch(t, &scanBasicString, basicString)
	swapDispatch(t, &scanBasicStringEscape, basicStringEscape)
